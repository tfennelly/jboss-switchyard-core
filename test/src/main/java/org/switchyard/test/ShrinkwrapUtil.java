...
public final class ShrinkwrapUtil {
     ...
     * @param groupId    Maven groupId
     * @param artifactId Maven artifactId.
     * @return The Maven Web artifact archive.
     */
    ...
    public static WebArchive getWebArchive(String groupId, String artifactId, String version) {
        ...
        String artifact = groupId + ":" + artifactId + ":" + version + ":war";
        Collection<WebArchive> resolvedArtifacts = DependencyResolvers.use(MavenDependencyResolver.class)
                .loadReposFromPom("./pom.xml").artifact(artifact).resolveAs(WebArchive.class);

        if (resolvedArtifacts.isEmpty()) {
            Assert.fail("Failed to resolve artifact '" + artifact + "'.");
        }

//        WebArchiveImpl webArchive = new WebArchiveImpl();
